...
<template>
    <div class="language-manager">
        <!-- 语言列表 -->
        <UiList  header="支持的语言" :items="languages" :selectedIndex="selectedIndex" :editable="true"
            :dark="dark" direction="horizontal" @select="handleSelectLanguage" @add="openAddLanguageModal"
            @remove="handleRemoveLanguage">
            <template #header-actions>
                <ui-button v-if="selectedIndex >= 0 && selectedIndex < languages.length && !isDefaultLanguage(languages[selectedIndex])"
                    class="set-default-btn" @click.stop="setAsDefaultLanguage(selectedIndex)">
                    设为默认
                </ui-button>
            </template>
            ...
            <template #item="{ item }">
                <div class="language-item">
                    ...
                    <ui-icon v-if="isDefaultLanguage(item)" class="default-icon" value="star" :tooltip="'默认语言'" />
                </div>
            </template>
        </UiList>

        <!-- 添加语言模态窗口 -->
        <UiModal v-model:visible="showAddLanguageModal" title="添加语言" okText="确认" cancelText="取消" :dark="dark"
