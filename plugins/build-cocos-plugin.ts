	...
	return {
		...
		closeBundle: async () => {
			try {
				...
				const buildDir = path.isAbsolute(outputDir) 
					? outputDir 
...
function processPackageJson(destPath: string): void {
	try {
		...
		const newPackageJson = {
			package_version: originalPackageJson.package_version || 2,
			version: originalPackageJson.version || "0.0.1",
			name: originalPackageJson.name || "Plugin Name",
			title: originalPackageJson.title || "Plugin Title",
			description: originalPackageJson.description || "Plugin Description",
			author: originalPackageJson.author || "Your Name",
			editor: originalPackageJson.editor || ">=3.4.2",
			main: originalPackageJson.main || "./dist/browser.cjs",
