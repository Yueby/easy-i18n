...
import { buildCocosPluginPackage } from './plugins/build-cocos-plugin';
...
export default defineConfig(({ mode }) => {
	...
	return {
		...
		plugins: [
			...
			cocosPanelConfig(),
			cocosPanelCss(),
			buildCocosPluginPackage({
				outputDir: 'D:/Develop/CocosCreator/extensions' // 可以自定义输出目录，支持相对路径和绝对路径
			})
		]
	};
});
